...
export default function TodoList() {
	...
	// REQUEST FUNCTIONS
	function getTaskListRequest() {
		const url = "http://svcy.myclass.vn/api/ToDoList/GetAllTask"
		Axios.get(url)
			.then(response => {
	...
	function addTaskRequest(newTask) {
		const url = "http://svcy.myclass.vn/api/ToDoList/AddTask"
	...
	function deleteTaskRequest(taskName) {
		const url = `http://svcy.myclass.vn/api/ToDoList/deleteTask?taskName=${taskName}`
		Axios.delete(url)
			.then(() => {
				getTaskListRequest()
			})
	...
	function doneTaskRequest(taskName) {
		const url = `http://svcy.myclass.vn/api/ToDoList/doneTask?taskName=${taskName}`
		Axios.put(url)
			.then(() => {
				getTaskListRequest()
			})
	...
	function rejectTaskRequest(taskName) {
		const url = `http://svcy.myclass.vn/api/ToDoList/rejectTask?taskName=${taskName}`
		Axios.put(url)
			.then(() => {
				getTaskListRequest()
			})
					...
					<li className="task-list__item" key={index}>
						...
						<div className="task-list__action">
							<span
								...
								onClick={() => doneTaskRequest(task.taskName)}
							>
								<svg
									aria-hidden="true"
									focusable="false"
							...
							<span
								className="task-list__btn"
								onClick={() => deleteTaskRequest(task.taskName)}
							>
								<svg
									aria-hidden="true"
									focusable="false"
					...
					<li className="task-list__item" key={index}>
						...
						<div className="task-list__action">
							<span
								className="task-list__btn"
								onClick={() => {
									rejectTaskRequest(task.taskName)
								}}
							>
								<svg
									aria-hidden="true"
